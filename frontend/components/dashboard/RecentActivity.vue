...
<template>
    <div class="bg-primaryWhite-500 dark:bg-secondaryDark-500 rounded-xl shadow-sm p-6">
        ...
        <div class="space-y-4">
            <div v-for="activity in activities" :key="activity.name" class="flex items-start space-x-3 p-3 rounded-lg border border-gray-100 dark:border-gray-900 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                <div>
                    <p className="text-sm font-medium text-fontBlack dark:text-fontWhite">
                        {{ activity.name }}
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-200">{{ activity.description }}</p>
